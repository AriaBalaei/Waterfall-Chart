...
function drawChart(data) {
  ...
  console.log(total)
  nodes.push(total)
  const bargraph = mainCanvas.selectAll("g").data(nodes).enter().append("g")

  const xAxisGroup = mainCanvas
    .append("g")
  ...
  const z = d3.scaleLinear().domain([max, 0]).range([0, graphHeight])
  //scaleBand
  const x = d3
    .scaleBand()
  ...
  const yAxis = d3.axisLeft(z).ticks(10, "$.2f")
  const xAxis = d3.axisBottom(x).tickFormat((d, i) => tickname[i])
...
function getY2(v1, v2, y1, h1, h2) {
  var y2 = 0
  if (v1 >= 0 && v2 < 0) y2 = y1
  if (v1 >= 0 && v2 >= 0) y2 = y1 - h2
  if (v1 < 0 && v2 >= 0) y2 = y1 + h1 - h2
  if (v1 < 0 && v2 < 0) y2 = y1 + h1
